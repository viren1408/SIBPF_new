...
import matplotlib.pyplot as pltfrom bdsf import spectralindex
import pandas as pd
import warnings
warnings.simplefilter(action='ignore')
from astroquery.vizier import Vizier
...
def Crossmatching(dir,region,file_path_image,file_path_spidx,file_path_TGSS,show_matches =True,get_spectral_index =True,get_candidates=True,get_pulsars =True ):
  ...
  save_file = str(dir)+'/'+str(region)+'.sav' #create .sav file
...
# Plot the matched sources on a RA-Dec plot, with marker sizes proportional to resolution
  ...
  if get_spectral_index == True:
    ...
    for i in range(len(spectral_index)):
      ...
      #spectral_in.append((i,Ra,Dec,matched_sources['Total_flux_TGSS'][i],matched_sources[' Total_flux'][i],matched_sources['Total_flux_NVSS'][i],slope,error,spidx,e_spidx))
    ...
    #column = ('sr.no','Ra_obs','Dec_obs','spectral_index','e_spectral_index','SPIDX','E_SPIDX')
...
# Plot the matched sources on a RA-Dec plot, with marker sizes proportional to resolution
  ...
  if get_spectral_index == True:
    ...
    for i in range(len(matched_sources)):
      ...
      spectral_in.append((i,Ra,Dec,slope,error,spidx,e_spidx))
  ...
  if get_pulsars == True :
    ...
    Pulsars = pd.DataFrame(pulsar_spectral_index,columns = column)
    filename =  str(dir)+'/'+str(region)+'/spectral_index_'+str(region)+'.csv'
    Pulsars.to_csv(filename, index=False)
    print(Pulsars)
